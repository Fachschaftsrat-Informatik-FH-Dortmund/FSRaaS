...
- [x] 5.2 Ausgabezeit zusätzlich ausweisen, wenn sie abweicht. Verifikation: Test `Ausweis der Ausgabezeit bei abweichender Öffnungszeit` mit den Werten des Max-Ophüls-Platzes (offen ab 08:00, Ausgabe ab 11:30) und der Hauptmensa (keine Abweichung). Erledigt: Anzeige in `CanteenScreen.tsx` neben der Öffnungszeit (Abschnittsüberschrift bei Mensa-Gruppierung, Fußzeile sonst); dabei eine veraltete `app/src/api/generated/schema.ts` neu erzeugt (`npm run generate` in `tools/contract-codegen`) — die generierte Datei ist ausgenommen von git und lag lokal hinter dem seit 1.3 erweiterten Vertrag zurück, wodurch `tsc` auf `oeffnungszeiten.ts` fehlschlug.
- [x] 5.3 Die drei Zustände je Mensa und Tag umsetzen — geöffnet mit Gerichten, geöffnet ohne Speiseplan, geschlossen. Verifikation: Tests `Geschlossen-Hinweis für geschlossene Mensa` und `Hinweis für geöffnete Mensa ohne Speiseplan`. Erledigt: `SortierKontext.istGeoeffnet` in `sortierung.ts` entscheidet den Abschnitts-`zustand` (`geschlossen`/`ohneSpeiseplan`/`gerichte`) bei Mensa-Gruppierung; `CanteenScreen.tsx` speist ihn aus `istGeoeffnet(oeffnungVon, datum)` und trägt dieselbe Unterscheidung im Fußbereich für die anderen Gruppierungen nach. `consolidate.ts`s `geschlossene`-Feld bleibt unverändert die reine „ohne Gerichte"-Liste, dokumentiert jetzt aber, dass die Geschlossen/Kein-Speiseplan-Unterscheidung erst in `sortierung.ts`/`CanteenScreen.tsx` entsteht (die Öffnungsangabe liegt der reinen Fachlogik in `consolidate.ts` nicht vor). Bestehende Tests, die eine leere Gerichtsliste implizit als „geschlossen" annahmen, sind auf explizite `mockOeffnung`-Angaben umgestellt (neuer Helfer `geschlossenAlleTage()`).
- [x] 5.4 Schließungsgrund und Zeitraum anzeigen. Verifikation: Test `Grund und Zeitraum einer Schließung` mit dem Fall der Mensa Süd (Betriebsferien bis 04.10.). Erledigt: `schliessungFuer` in `oeffnungszeiten.ts` verknüpft `grund` des angezeigten Tages mit dem passenden Zeitraum aus `schliesstage`; `CanteenScreen.tsx` hängt den Text (unverändert wie geliefert, mit Enddatum in Kurzform) an den Geschlossen-Hinweis an — in der Abschnittsüberschrift (Mensa-Gruppierung) wie im Fußbereich (andere Gruppierungen).
- [x] 5.5 Wiedereröffnungshinweis auf Öffnungsvorschau und Schließtage umstellen. Verifikation: Test `Wiedereröffnungshinweis an der geschlossenen Mensa`, einschließlich des Falls einer Mensa ohne Speiseplan. Erledigt: Die Umstellung stand bereits mit Abschnitt 2 (Backend) — `MensaQuelle.NaechsteOeffnung` sucht ausschließlich über `Forecast`/`Week`/`Closures`, nie über den Speiseplan; die App zeigt das durchgereichte Feld unverändert an. Ergänzt: App-Test für den Fall „Mensa ohne Speiseplan" (Requirement-Scenario), der bislang nur implizit mitlief.
- [x] 5.6 Öffnungszeit für geschlossene Mensen unterdrücken, für geöffnete ohne Speiseplan anzeigen. Verifikation: Test `Keine Öffnungszeit für geschlossene Mensa`. Erledigt: bereits mit 5.3 umgesetzt — die `zeit`/`ausgabe`-Berechnung in `CanteenScreen.tsx` schließt nur `art === 'geschlossen'` aus, `'ohneSpeiseplan'` behält seine Öffnungszeit; ebenso der Fußbereich (`wirklichGeschlossen` statt der früheren pauschalen `geschlossene`-Liste). Test „zeigt für eine geöffnete Mensa ohne Speiseplan weiterhin ihre Öffnungszeit" ergänzt.
- [x] 5.7 Öffnungszeit an der Abschnittsüberschrift auch im Fall „geöffnet ohne Speiseplan" führen. Verifikation: Test `Öffnungszeit an der Mensa-Abschnittsüberschrift`. Erledigt: ebenfalls bereits mit 5.3 in der Anzeigelogik enthalten; Test für das Scenario „Geöffnete Mensa ohne Speiseplan" im Requirement-benannten Describe-Block ergänzt.
- [x] 5.8 Wochenend-Überspringen auf die Öffnungsangabe umstellen. Verifikation: Test `Überspringen geschlossener Wochenendtage` mit einem samstags geöffneten Standort ohne Speiseplan. Erledigt: `CanteenScreen.tsx` speist `naechsterTag` (`tageswahl.ts`) jetzt mit `istOffenAn` (aus `istGeoeffnet`/Öffnungsangaben) statt mit dem bisherigen `hatAngebot` (Speiseplan-Zwischenspeicher der App); `tageswahl.ts`s Parameter und Dokumentation entsprechend umbenannt. Die Speiseplan-Vorablade-Wirkung des Vorlaufs bleibt als reine Performance-Maßnahme erhalten.
- [x] 5.9 Chip-Leiste um den Abschnitt der geöffneten Mensa ohne Speiseplan ergänzen. Verifikation: Test `Nicht auswählbare Chips ohne sichtbaren Abschnitt`. Erledigt: bereits mit 5.3 in der Chip-Logik enthalten — `chips` in `CanteenScreen.tsx` entsteht aus `struktur.abschnitte`, das für `art === 'ohneSpeiseplan'` wie für `'geschlossen'` einen Abschnitt führt und damit stets `sichtbareIds` zugehört. Test für das Scenario „Geöffnete Mensa ohne Speiseplan bei Mensa-Gruppierung" ergänzt.
...
- [x] 6.2 Allergene und Zusatzstoffe am Gericht unterscheidbar ausweisen. Verifikation: Test `Gerichtsangaben — Kategorie, Bezeichnung, Preise, Zusatzstoffe`. Erledigt: `reineZusatzstoffe()` in neuem `gerichtsangaben.ts` rechnet `allergene` aus `zusatzstoffe` heraus (dessen Bedeutung bleibt die alte, umfassendere); `GerichtKarte` (`CanteenScreen.tsx`) und die Gerichtskarte in `CanteenAllScreen.tsx` zeigen beide Zeilen getrennt und beschriftet („Allergene:" / „Zusatzstoffe:").
...
- [ ] 9.4 Offene Frage in `specs/open-questions.md` („Woher stammt die Mensa-Liste …") auf INT-020 nachführen. Verifikation: der Eintrag nennt die neue Quelle und das Entscheidungsdatum.
- [ ] 9.5 Offene Fragen in `openspec/specs/canteen/spec.md` nachführen: die Frage nach der führenden Öffnungszeit-Quelle als am 2026-09-22 neu entschieden durchstreichen, den Punkt „Weitere Gruppierungs-/Sortier-Bausteine" um die aufgenommene CO₂-Klasse ergänzen. Verifikation: beide Einträge geben den Stand nach diesem Change wieder.
