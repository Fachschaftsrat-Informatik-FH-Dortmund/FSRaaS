...
### Ehemals MENSA-N-020: Aktualisierungszeitplan des Speiseplan-Zwischenspeichers

Ursprünglicher Text: „Der Speiseplan-Zwischenspeicher des Backends (Capability `backend-and-api`, API-F-076) sollte vor der morgendlichen und vor der mittäglichen studentischen Nutzungsspitze je einmal sowie einmal zeitnah nach dem Ende des Mensabetriebs aus INT-015 aufgefrischt worden sein, damit der in der App angezeigte Plan die tatsächliche Tagesausgabe abbildet, bevor die meisten Studierenden nachsehen, und eine kurzfristige Änderung des Folgetagsangebots noch am selben Abend sichtbar wird." Herkunft: NEU.

Status: entfallen (entschieden 2026-09-22, umgesetzt 2026-09-24 mit dem Change `mensa-api-abloesung`). Grund: Der serverseitige Speiseplan-Zwischenspeicher entfällt mit der Ablösung von INT-015 ersatzlos — INT-020 ist selbst ein Zwischenspeicher im Verantwortungsbereich des FSR. Ein zweiter davor brächte keinen Gewinn, sondern eine weitere Stelle, an der ein veralteter Stand hängen bleiben kann; genau dieser Fall ist bei der Quelle selbst messbar aufgetreten. Damit entfällt auch der Zeitplan, nach dem er aufzufrischen wäre. Ersetzt durch „Mensa-Daten durchreichen statt zwischenspeichern" (Capability `backend-and-api`); meldet die Quelle einen auffällig alten Stand, greift „Altershinweis bei veralteten Daten der Mensa-Schnittstelle" (Capability `data-and-storage`). Siehe `specs/decisions/0021-mensa-api-des-fsr-ohne-eigenen-zwischenspeicher.md`.
...
**Aktualität des Speiseplans** (Requirement „Neuladen durch Herunterziehen"). Ergänzt am 2026-09-04, **nachgeführt am 2026-09-24**. Der Speiseplan ist die meistgenutzte Ansicht der App, und Studierende sehen typischerweise zu zwei Zeitpunkten nach: morgens vor dem Aufstehen und kurz vor der Essenszeit, bevor sie zur Mensa gehen. Zwei getrennte Maßnahmen:

- **Serverseitig**: Das Backend reicht den Tagesplan je Anfrage aus INT-020 durch und beachtet dabei die von der Quelle gesetzte Gültigkeitsdauer (Capability `backend-and-api`, Requirement „Mensa-Daten durchreichen statt zwischenspeichern"). Bis zum 2026-09-24 hielt es stattdessen einen eigenen Zwischenspeicher, den ein Zeitplan vor den Nutzungsspitzen und nach Mensaschluss auffrischte (API-F-076 über MENSA-N-020); beides ist mit der Ablösung von INT-015 entfallen, weil INT-020 selbst ein Zwischenspeicher im Verantwortungsbereich des FSR ist — die Nutzungsspitzen halten ihn ohnehin warm. Meldet die Quelle dennoch einen überalterten Stand, macht der Altershinweis das sichtbar (Capability `data-and-storage`). Die App ruft die Mensa-Quelle weiterhin nie selbst ab (Capability `architecture`, Requirement „News und Mensa-Speisepläne über das Backend").
- **Clientseitig**: Zusätzlich zur automatischen Aktualität kann die Nutzerin die Liste durch Herunterziehen neu laden — die feature-spezifische Ausprägung der querschnittlichen Aktualisierungsgeste (Capability `ux-and-theming` UX-F-160). Das Herunterziehen lädt die Tagespläne der gewählten Mensen neu über das Backend. Der Altershinweis (Abschnitt „Offline-Verhalten") spiegelt danach den Stand, den die Quelle meldet — seit dem 2026-09-24 auch bei bestehender Netzverbindung, wenn dieser Stand überaltert ist.
...
Siehe Requirement „Zielzeitpunkt für den Lieblingsgerichte-Abgleich" (vormals MENSA-N-010) oben. „Aktualisierungszeitplan des Speiseplan-Zwischenspeichers" (vormals MENSA-N-020) ist am 2026-09-24 entfallen, siehe „Entfallene Anforderungen (historisch)".
...
- ~~Welche der beiden Öffnungszeit-Quellen führend ist — die Schnittstelle (INT-015, `openings/all`) oder die gepflegten Stammdaten.~~ Entschieden am 2026-09-03 (Roadmap-Schritt 4): Die gepflegten Stammdaten sind führend; `openings/all` antwortete bei der Verifikation mit HTTP 500 und wird nicht genutzt (Capability `integrations` INT-015). ~~Neu entschieden am 2026-09-22: Mit der Ablösung durch die FSR-eigene Mensa-API (`mensa.fb4.it`) liefert die Schnittstelle selbst verlässliche Öffnungszeiten, Ausgabezeiten und Schließtage; die Handpflege in den Stammdaten entfällt ersatzlos, weil sie am selben Tag als verfallen belegt wurde (Hauptmensa freitags gepflegt `11:30 - 14:00`, tatsächlich `11:30 - 14:15`) — siehe `specs/decisions/0021-mensa-api-des-fsr-ohne-eigenen-zwischenspeicher.md`; der Registereintrag folgt mit dem Archivieren des Change `mensa-api-abloesung`.~~
...
- **Weitere Gruppierungs-/Sortier-Bausteine.** Die aktuellen Requirements zu Sortierung und Gruppierung decken die am 2026-09-04 benannten Regelfälle, die naheliegenden Ergänzungen (Preis, alphabetisch) sowie seit 2026-09-22 die CO₂-Klasse aus der FSR-eigenen Mensa-API (Change `mensa-api-abloesung`) als Sortierkriterium und Gruppierungs-Baustein ab. Ob weitere Bausteine sinnvoll sind — etwa Gruppierung nach Kennzeichnung (vegan/vegetarisch/Fleisch) oder Sortierung nach Entfernung/Öffnungsstatus der Mensa — ist offen und nach erster Nutzung zu prüfen, FSR FB4.
