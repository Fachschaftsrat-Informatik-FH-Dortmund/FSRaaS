...
public class CustomEventActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.inputTimeStart.setText(now.format(timeFormatter));

        binding.inputTimeEnd.setText(now.plusMinutes(45).format(timeFormatter));
        ...
        binding.spinnerWeekday.setSelection(day);
    }
        ...
        switch (item.getItemId()) {
            case R.id.speichern:
                if (!binding.inputCourseName.getText().toString().isEmpty()) {
                    ...
                    newEvent.setDay(getSelectedDay());
                    newEvent.setName(binding.inputCourseName.getText().toString());
                    newEvent.setCourseType(binding.editTextEventType.getText().toString());
                    newEvent.setLecturerId(binding.inputLecturerId.getText().toString());
                    newEvent.setLecturerName(binding.inputLecturerName.getText().toString());
                    newEvent.setRoomId(binding.inputRoom.getText().toString());
                    newEvent.setStartTime(LocalTime.parse(binding.inputTimeStart.getText().toString()));
                    newEvent.setEndTime(LocalTime.parse(binding.inputTimeEnd.getText().toString()));

                    Futures.addCallback(
                        timetableEventDao.insertAsync(newEvent),
                        CallbackUtil.onSuccess(result -> {
                            Intent returnIntent = new Intent();
                            setResult(RESULT_OK, returnIntent);
                            finish();
                        }),
                        ContextCompat.getMainExecutor(CustomEventActivity.this)
                    );
                } else {
