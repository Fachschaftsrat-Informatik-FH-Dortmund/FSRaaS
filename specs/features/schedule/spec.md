...
| SCHED-F-280 | Das System muss der Nutzerin ermöglichen, aus den nach SCHED-F-400 abgerufenen Wahlpflichtmodulen eines für die Planung auszuwählen. | NEU |
...
- **Neu (2026-08-26), aus derselben Live-Prüfung:** „Lern- und Arbeitstechniken" wird parallel in zwei Räumen mit identischem `studentSet` angeboten (`courseId 411031`, siehe `platform/integrations.md` INT-002) — die Rohdaten allein lassen nicht erkennen, welcher Raum für eine einzelne Studierende gilt. Ohne Behandlung würde SCHED-F-230 hier einen Scheinkonflikt melden, da beide Varianten zeitgleich liegen. Vor Umsetzung zu klären, ob ein weiteres Feld oder eine externe Zuordnung (z. B. alphabetisch nach Nachname) die Zuordnung trägt, oder ob die App beide Varianten unverbunden nebeneinander anzeigen muss.
