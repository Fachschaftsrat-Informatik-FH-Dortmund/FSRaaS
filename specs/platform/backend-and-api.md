...
implemented_in:
  ...
  - backend/src/Fb4.Backend/Infrastructure/Mensa # API-F-070, API-F-075 (INT-015-Abruf, Zwischenspeicher, Aktualisierungs-Job); API-F-076 (Ortszeit-Zeitplan, SpeiseplanAbrufZeitplan); API-N-110/N-120, API-F-260
...
| API-F-076 | Das System muss den Speiseplan-Zwischenspeicher (API-F-070) so aus INT-015 auffrischen, dass eine Aktualisierung vor der morgendlichen und vor der mittäglichen studentischen Nutzungsspitze sowie zeitnah nach dem Ende des Mensabetriebs abgeschlossen ist, zusätzlich zu einem regelmäßigen Grundintervall über den Tag. | NEU |
...
Zu API-F-076: Der Speiseplan-Zwischenspeicher wurde in Roadmap-Schritt 4 in einem festen 6-Stunden-Intervall ab Prozessstart aufgefrischt. Das trifft die studentischen Nutzungsspitzen nur zufällig. Da der Speiseplan die meistgenutzte Ansicht der App ist und Studierende typischerweise morgens vor dem Aufstehen und kurz vor der Essenszeit nachsehen (`../features/canteen/spec.md` MENSA-N-020), wird der Job auf Ortszeit-Läufe umgestellt, die vor beiden Fenstern liegen (z. B. gegen 05:30 und 10:00 Uhr), sowie einen Lauf **zeitnah nach dem Ende des Mensabetriebs** (z. B. gegen 15:30 Uhr): Änderungen am Angebot des Folgetags — oder eine kurzfristige Umstellung — trägt das Studierendenwerk erfahrungsgemäß gegen Ende des laufenden Betriebstags in die Quelle ein; ein Lauf direkt danach macht sie am selben Abend statt erst am nächsten Morgen sichtbar. Ergänzt wird das um ein regelmäßiges Grundintervall über den Tag, damit auch dazwischenliegende Änderungen und Abenddaten fließen. Der Lauf bleibt ein fester, sparsamer Zeitplan im Sinne von `non-functional.md` NFR-N-090 — kein fortlaufendes Polling, keine nachfragegesteuerte Auslösung (dazu die offene Frage in `../features/canteen/spec.md` Abschnitt 13). Die konkreten Uhrzeiten und die Ortszeitzone sind konfigurierbar (Vorgaben im Code), nicht fest verdrahtet. Die Fehler-Isolation je Lauf (API-N-110) und die Resilienz je INT-015-Aufruf (API-N-120) gelten unverändert.
