...
| Kategoriezuordnung im Speiseplan | MENSA, Rohdaten aus INT-015 (`counterNames`, `counter`, `category`) | Nicht jeder Verbrauchsort liefert eine anzeigbare Kategorie; ein roher Zahlencode als Überschrift (Verhalten der Android-Alt-App) ist für die Nutzerin bedeutungslos, ein weggefallenes Gericht ein stiller Datenverlust | Regelfall mit `counterNames{de,en}`; nur `counter` ohne `counterNames`; `Beilagen` (getrennt, ans Ende — MENSA-F-040); Food Fakultät (Kennung 474): `counter` fehlt, `counterNames` `null`, nur nicht auflösbarer `category`-Code → alle Gerichte in einer Sammelgruppe ohne Überschrift, keines fällt weg (MENSA-F-160) |
| Zusammenfassung der Gerichtsliste über mehrere Mensen | MENSA, je Mensa gelieferte Tagespläne (`features/canteen/spec.md` MENSA-F-012 bis F-018) | Neue Fachlogik: dieselben Gerichte an mehreren gewählten Mensen werden zu einem Eintrag zusammengezogen; falsche Auflösung führt zu doppelten Einträgen oder falschen Preisen | Gericht nur an einer Mensa; Gericht an zwei Mensen mit gleichem Preis → ein Eintrag, beide Mensen genannt; Gericht an zwei Mensen mit **abweichendem** Preis → ein Eintrag, Angaben der maßgeblichen Mensa (aktive Mensa wenn anbietend, sonst erste in der Reihenfolge — MENSA-F-018), keine Aufspaltung, kein Durchschnitt; aktive Mensa führt das Gericht nicht → maßgeblich ist die erste anbietende; Wochenende ohne Angebot beim Blättern übersprungen (MENSA-F-044) |
