...
**Angepinnter Termin**
Ein Termin des persönlichen Stundenplans, dessen Zeitslot festgehalten ist: Die Planung darf ihn weder auf einen anderen Gruppen-Slot umlegen noch entfernen. Betrifft das *Wann* — abzugrenzen von der Pflicht-Veranstaltung, die das *Ob* festhält. Typischer Anlass: Die eigene Lerngruppe geht zu genau diesem Termin. Auftritt: `openspec/specs/schedule/spec.md`.
...
**Gruppenkennung**
Vom Studierenden angegebene, rein lokal verwendete Kennung aus genau einem Großbuchstaben und mindestens einer Ziffer, z. B. `C8`. Muster `^[A-Z][0-9]+$`. Beide Teile sind verpflichtend: Fünf der 21 im FBWS-Bestand vorkommenden `studentSet`-Werte tragen eine Zahl an einer Bereichsgrenze, an der sie mitentscheidet. Wird nie an einen Server übertragen (siehe `openspec/specs/security-and-privacy/spec.md`), sondern lokal gegen das `studentSet` eines Termins abgeglichen. Auftritt: `openspec/specs/schedule/spec.md`.

*Fassungsgeschichte, damit die Runde nicht erneut gedreht wird:* Das Muster war bis zum 2026-09-04 `^[A-Z][0-9]+$`, wurde dann auf eine freiwillige Ziffer erweitert und am 2026-09-06 zurückgeführt — die Erweiterung stützte sich auf die Annahme, die Zahl werde „so gut wie nie gebraucht", die der eigene Datenbefund widerlegt.
...
**Kriterienrangfolge**
Die von der Nutzerin geordnete Liste der Kriterien, nach denen die Stundenplan-Planung mehrere konfliktfreie Termine reiht (Uni-Tage, Anwesenheitszeit, gleichmäßige Woche, Abstand zwischen Lerneinheiten, Vorbereitungszeit, Abstand Vorlesung–Übung, Abwechslung, Zeitfenster). Einzelne Kriterien sind abschaltbar. Ausgewertet wird streng der Reihe nach: Das oberste eingeschaltete Kriterium entscheidet, erst bei Gleichstand zählt das nächste. Ersetzt seit dem 2026-09-06 den vormaligen Begriff *Optimierungsmodus*, der als benannte Voreinstellung fortlebt. Auftritt: `openspec/specs/schedule/spec.md`.
...
Ein Kandidat der Planungsauswahl, der als „Pflicht" markiert ist: Die Veranstaltung muss im Plan enthalten bleiben, welcher ihrer Termine gewählt wird, bleibt der Planung überlassen. Betrifft das *Ob* — abzugrenzen vom angepinnten Termin, der das *Wann* festhält. Auftritt: `openspec/specs/schedule/spec.md`.

**Prüfungsbestand**
Die Menge der offiziellen Prüfungstermine, die das Backend aus dem Raumplan ableitet: Einträge mit `eventType: "Event"`, deren Bezeichnung dem Muster `Prüfung <Modulnummer> <Bezeichnung>` folgt. Seit dem 2026-09-06 die einzige Quelle; der vormalige halbautomatische Excel-Import aus dem Hochschul-Intranet (ehemals INT-013) ist entfallen. Auftritt: `openspec/specs/backend-and-api/spec.md`, `openspec/specs/schedule/spec.md`, `openspec/specs/integrations/spec.md` (INT-009).
...
**Voreinstellung (Planung)**
Eine benannte, fertige Kriterienrangfolge für die Stundenplan-Planung — *Zeit an der Uni*, *Fahrten zur Uni*, *Ausgeglichene Woche*, *Abstand zwischen Lerneinheiten*, *Vorbereitungszeit*. Eine Voreinstellung schaltet genau die Kriterien ein, die sie ausmachen, und lässt die übrigen abgeschaltet; die Reihenfolge bleibt danach von Hand änderbar. Nicht zu verwechseln mit den Sortier-/Gruppier-Presets des Mensaplans. Auftritt: `openspec/specs/schedule/spec.md`.
