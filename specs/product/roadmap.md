...
Zu Schritt 6: Der Stundenplan-Raumabgleich (SCHED-F-410 bis F-450) gehört fachlich zum Stundenplan, wird aber hier umgesetzt, weil er den Raumplan-Zwischenspeicher aus RAUM voraussetzt. Der vorab geforderte Spike auf `../../openspec/specs/integrations/spec.md` INT-009 ist am 2026-09-07 durchgeführt (Change `int-009-spike-befund`, Issue #28) und **negativ ausgefallen**: Der Raumplan bildet kurzfristige Ausfälle nicht ab — er führt selbst gesetzliche Feiertage als belegt (Pfingstmontag und Fronleichnam 2026 tragen exakt so viele Kursinstanzen wie der gleiche Wochentag der Folgewoche). Wie vorgesehen bleiben Ansicht laufender Veranstaltungen und Abgleichhinweis damit auf den Sollplan beschränkt; ob die sechs Abgleich-Requirements in diesem Zuschnitt noch tragen, ist eine offene Entscheidung. Zwei weitere Befunde des Spikes wirken unmittelbar in diesen Schritt hinein: Der Abruf muss den Zeitraum über die undokumentierten Parameter `From`/`To` ausdrücklich setzen, sonst liefert der Endpunkt nur sieben Tage; und 59 % der Kursinstanzen tragen die Pseudo-Raumkennung `*` und nennen ihren Raum nur im Namensfeld — wer das übergeht, meldet rund ein Drittel der belegten Zeit als frei.
