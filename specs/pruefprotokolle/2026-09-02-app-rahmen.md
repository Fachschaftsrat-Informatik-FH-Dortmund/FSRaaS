...
| Build | `npx expo export --platform android` erfolgreich (JS-Bundle ~2,86 MB Hermes); `tsc`, ESLint, Jest (177 Tests), spec-check (4/4), audit-ci — alles grün |

## 1. Automatisiert abgedeckt (hier nur zur Einordnung)

| Anforderung | Test |
|---|---|
| UX-F-020 / UX-F-030 | `app/src/theme/ThemeProvider.test.tsx`, `appearanceMode.test.ts`; `app/src/areas/more/screens/MoreScreen.test.tsx` (Einträge im Dunkelmodus nach Farbsystem) |
